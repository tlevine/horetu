...
def test_one(function, argv, result):
    ...
    f = one(None, None, parser, function)
    observed = f(parser.parse_args(argv))
    assert observed == result

class FakeParser(object):
    def __init__(self):
        self.optional_flags = set()
    def add_argument(self, *args, **kwargs):
        for a in args:
            if a.startswith('-'):
                self.optional_flags.add(a)
...
@pytest.mark.parametrize('function, optional_flags', flag_cases)
def test_flags(function, optional_flags):
    parser = FakeParser()
